...
Page({
     ...
     getHotelInfo: function () {
          wx.getStorage({
               ...
               success: (res) => {
                    let images = []
                    res.data.pic.split(',').forEach(file => {
                         images.push({ imgSrc: 'http://192.168.0.102:9527/imagesWeb/' + file, webUrl: '' })
                    })
                    that.setData({
                         homeAdvertises: images
                    })
               },
               fail: res => {
                    let that = this
     ...
     searchEvent: function () {
          if (location == '定位中...') {
               ...
               wx.navigateTo({
                    url: '../../pages/hotelDetail/hotelDetail?location=' + this.data.location,
               })
          }
     },
