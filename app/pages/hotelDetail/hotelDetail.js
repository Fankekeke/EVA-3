...
Page({
     ...
     getRoomList: function () {
          let that = this
          http.get('roomList').then((res) => {
               let roomList = []
               res.data.forEach(item => {
                    if (item.roomCount != item.useRoom) {
                         let room = {
                              image: 'http://192.168.0.102:9527/imagesWeb/' + item.pic.split(',')[0],
                              name: item.typeName,
                              service: item.size +" / "+item.remaker,
                              price: item.price,
                              bed: item.bed,
                              remaker: item.remaker,
                              type: item.type
                         }
                         roomList.push(room)
                    }
     ...
     bookRoom: function (e) {
          ...
          wx.navigateTo({
               url: '../bookHotel/bookHotel?price=' + room.price + '&hotelName=' + this.data.hotelName + '&roomName=' + room.name + '&startDate=' + startDate + '&endDate=' + endDate + '&bed=' + room.bed + '&remaker=' + room.remaker+ '&type=' + room.type,
          })
     },
